...
import type { QQBotProactiveQuietHours, ResolvedQQBotAccount } from "./types.js";
import {
  getAccessToken,
...
function getZonedDateParts(source: Date, timeZone: string): { hour: number } {
...
function isWithinQuietHours(source: Date, quietHours: NormalizedProactiveQuietHours): boolean {
  const hour = getZonedDateParts(source, quietHours.timezone).hour;
