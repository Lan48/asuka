...
export async function sendC2CMessage(
  ...
  return sendAndNotify(accessToken, "POST", `/v2/users/${openid}/messages`, body, { text: content, targetId: openid });
}
...
export async function sendGroupMessage(
  ...
  const msgSeq = msgId ? getNextMsgSeq(msgId) : 1;
  const body = buildMessageBody(content, msgId, msgSeq, messageReference);
  return sendAndNotify(accessToken, "POST", `/v2/groups/${groupOpenid}/messages`, body, { text: content, targetId: groupOpenid });
}
...
function buildPlainTextProactiveMessageBody(content: string): Record<string, unknown> {
  if (!content || content.trim().length === 0) {
    throw new Error("主动消息内容不能为空 (content is empty)");
  }
  return { content, msg_type: 0 };
}

function shouldRetryProactiveC2CAsPlainText(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error);
  return /\/v2\/users\/.+\/messages/.test(message) && /(11255|invalid request)/i.test(message);
}
...
export async function sendProactiveC2CMessage(
  ...
  try {
    return await sendAndNotify(accessToken, "POST", `/v2/users/${openid}/messages`, body, { text: content });
  } catch (error) {
    if (!currentMarkdownSupport || !shouldRetryProactiveC2CAsPlainText(error)) {
      throw error;
    }
    console.warn(`[qqbot-api] sendProactiveC2CMessage: markdown delivery failed, retrying as plain text for ${openid}`);
    const plainTextBody = buildPlainTextProactiveMessageBody(content);
    return await sendAndNotify(accessToken, "POST", `/v2/users/${openid}/messages`, plainTextBody, { text: content, targetId: openid });
  }
}
...
export async function sendC2CFileMessage(accessToken: string, openid: string, fileBase64?: string, fileUrl?: string, msgId?: string, fileName?: string, localFilePath?: string): Promise<MessageResponse> {
  ...
  return sendC2CMediaMessage(accessToken, openid, uploadResult.file_info, msgId, undefined,
    { targetId: openid, mediaType: "file", mediaUrl: fileUrl, mediaLocalPath: localFilePath ?? fileName });
}
...
export async function sendC2CVideoMessage(accessToken: string, openid: string, videoUrl?: string, videoBase64?: string, msgId?: string, content?: string, localPath?: string): Promise<MessageResponse> {
  ...
  return sendC2CMediaMessage(accessToken, openid, uploadResult.file_info, msgId, content,
    { text: content, targetId: openid, mediaType: "video", ...(videoUrl ? { mediaUrl: videoUrl } : {}), ...(localPath ? { mediaLocalPath: localPath } : {}) });
}
